...
def optimize_tracks(track3d: stereo4d_code.utils.Track3d):
  """
  ...
  motion = stereo4d_code.utils.get_scene_motion_2d_displacement(track3d)
...
def load_rgbd_cam_from_pkl(vid, root_dir, hfov=120, new_imw=1, new_imh=1):
  ...
  for fid in range(nfr):
    ...
    depth = stereo4d_code.utils.flow_to_depth(
        -flows[fid]['fwd'], input_dict['left']['camera'][fid].get_hfov_deg(), 0.063
    )  # pytype: disable=attribute-error
  
    # Remove occluded points
    flow_bwd_warp = stereo4d_code.utils.inverse_warp(flow_bwd, flow_fwd)
...
def save_3d_track_vis(
    track3d: stereo4d_code.utils.Track3d,
    rgbs: np.ndarray,
    depth: np.ndarray, 
    save_root: str,
    vid: str,
  ...
  motion_mag = stereo4d_code.utils.get_scene_motion_2d_displacement(track3d)
  ...
  dyna_track_plt = stereo4d_code.utils.plot_3d_tracks_plt(
      rgbs,
      track3d_dynamic_sort_sampled,
  )
  media.write_video(
...
def plot_3d_tracks_with_camera(track3d: stereo4d_code.utils.Track3d, depth, rgb, axes=None, tracks_leave_trace=32):
